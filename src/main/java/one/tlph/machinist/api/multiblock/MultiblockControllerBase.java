...
public abstract class MultiblockControllerBase implements IMultiblockValidator {
	...
	public void attachBlock(IMultiblockPart part) {
		...
		if(!connectedParts.add(part))
			Machinist.logger.warn("[%s] Controller %s is double-adding part %d @ %s. This is unusual. If you encounter odd behavior, please tear down the machine and rebuild it.",
	...
	public final void updateMultiblockEntity() {
		...
		if(WORLD.isRemote) {
			...
			if(minimumCoord != null && maximumCoord != null &&
				...
				int minChunkX = new ChunkPos(this.minimumCoord).getXStart();
				int minChunkZ = new ChunkPos(this.minimumCoord).getZStart();
				int maxChunkX = new ChunkPos(this.maximumCoord).getXEnd();
				int maxChunkZ = new ChunkPos(this.maximumCoord).getZEnd();
	...
	public boolean shouldConsume(MultiblockControllerBase otherController) {
		...
		if(res < 0) { return true; }
			...
			Machinist.logger.warn("[%s] Encountered two controllers with the same reference coordinate. Auditing connected parts and retrying.", this.WORLD.isRemote ? "CLIENT" : "SERVER");
			auditParts();
	...
	private void auditParts() {
		...
		Machinist.logger.warn("[%s] Controller found %d dead parts during an audit, %d parts remain attached", this.WORLD.isRemote ? "CLIENT" : "SERVER", deadParts.size(), this.connectedParts.size());
	}
		...
		for(int x = getMinimumCoord().getX(); x <= this.getMaximumCoord().getX(); x++)
			for(int y = getMinimumCoord().getY(); y <= this.getMaximumCoord().getY(); y++)
				for(int z = getMinimumCoord().getZ(); z <= this.getMaximumCoord().getZ(); z++) {
					...
					this.WORLD.markAndNotifyBlock(p, null, WORLD.getBlockState(p), WORLD.getBlockState(p), 3);
				}
	}
