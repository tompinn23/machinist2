...
public final class MultiblockRegistry implements IMultiblockRegistry {
     ...
     */
    @Override
    public void onPartAdded(final IWorld world, final IMultiblockPart part) {
     ...
     */
    @Override
    public void onPartRemovedFromWorld(final IWorld world, final IMultiblockPart part) {
     ...
     */
    @Override
    public void addDeadController(final IWorld world, final MultiblockControllerBase controller) {

        if (this._registries.containsKey(world))
            this._registries.get(world).addDeadController(controller);
        else
            Machinist.logger.warn("Controller %d in world %s marked as dead, but that world is not tracked! Controller is being ignored.", controller.hashCode(), world);
    }
     ...
     */
    @Override
    public void addDirtyController(final IWorld world, final MultiblockControllerBase controller) {
    ...
    private MultiblockRegistry() {
        this._registries = new HashMap<IWorld, MultiblockWorldRegistry>(2);
    }

    private HashMap<IWorld, MultiblockWorldRegistry> _registries;
