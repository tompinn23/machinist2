package tjp.machinist.api.multiblock.rectangular;
...
public abstract class RectangularMultiblockControllerBase extends MultiblockControllerBase {
	...
	protected boolean isMachineWhole(IMultiblockValidator validatorCallback) {
		...
		if (maxXSize > 0 && deltaX > maxXSize) { validatorCallback.setLastError("message.machinist.multiblock.validation.machine_too_large", maxXSize, "X"); return false; }
		if (maxYSize > 0 && deltaY > maxYSize) { validatorCallback.setLastError("message.machinist.multiblock.validation.machine_too_large", maxYSize, "Y"); return false; }
		if (maxZSize > 0 && deltaZ > maxZSize) { validatorCallback.setLastError("message.machinist.multiblock.validation.machine_too_large", maxZSize, "Z"); return false; }
		if (deltaX < minXSize) { validatorCallback.setLastError("message.machinist.multiblock.validation.machine_too_small", minXSize, "X"); return false; }
		if (deltaY < minYSize) { validatorCallback.setLastError("message.machinist.multiblock.validation.machine_too_small", minYSize, "Y"); return false; }
		if (deltaZ < minZSize) { validatorCallback.setLastError("message.machinist.multiblock.validation.machine_too_small", minZSize, "Z"); return false; }
				...
				for(int z = minZ; z <= maxZ; z++) {
					...
					if(te instanceof RectangularMultiblockTileEntityBase) {
						...
						if(!myClass.equals(part.getMultiblockControllerType())) {
							...
							validatorCallback.setLastError("message.machinist.multiblock.validation.invalid_part", x, y, z);
							return false;
						}
					}
					else {
					...
					if(extremes >= 2) {
						...
						if (!isPartValid) {
							...
							if (null == validatorCallback.getLastError())
								validatorCallback.setLastError("message.machinist.multiblock.validation.invalid_part_for_frame", x, y, z);
						...
						if(y == maxY) {
							...
							if (!isPartValid) {
								...
								if (null == validatorCallback.getLastError())
									validatorCallback.setLastError("message.machinist.multiblock.validation.invalid_part_for_top", x, y, z);
							...
							if (!isPartValid) {
								...
								if (null == validatorCallback.getLastError())
									validatorCallback.setLastError("message.machinist.multiblock.validation.invalid_part_for_bottom", x, y, z);
							...
							if (!isPartValid) {
								...
								if (null == validatorCallback.getLastError())
									validatorCallback.setLastError("message.machinist.multiblock.validation.invalid_part_for_sides", x, y, z);
						...
						if (!isPartValid) {
							...
							if (null == validatorCallback.getLastError())
								validatorCallback.setLastError("message.machinist.multiblock.validation.reactor.invalid_part_for_interior", x, y, z);
